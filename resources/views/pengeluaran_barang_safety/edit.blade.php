...
@section('title', 'Edit Pengeluaran Safety')

@section('content')
<div class="container">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">EDIT PENGELUARAN ALAT SAFETY</h1>
    </div>
    ...
    <div class="card bg-white shadow mb-4">
        <div class="card-body">
            ...
            <form action="{{ route('pengeluaran_barang_safety.update', Crypt::encrypt($pengeluaran->id)) }}"method="POST"enctype="multipart/form-data">
                    ...
                    @foreach($pengeluaran->items as $index => $item)
                        <div class="item-row row mb-2 align-items-end">
                            <div class="col-md-5">
                                ...
                                <select name="items[{{ $index }}][barang_safety_id]" class="form-control select2-item" required>
                                    <option value="">-- Pilih Alat --</option>
                                    @foreach($barang_safety as $b)
                                        <option value="{{ $b->id }}"
                                            {{ $item->pivot->barang_safety_id == $b->id ? 'selected' : '' }}>
                ...
                <div class="mt-4">
                    ...
                    <a href="{{ route('pengeluaran_barang_safety.index') }}" class="btn btn-secondary">
                        Kembali
                    </a>
                </div>
            </form>
